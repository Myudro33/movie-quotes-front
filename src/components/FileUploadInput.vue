<template>
  ...
  >
    <img v-if="props.edit" :src="props.image" class="xs:h-28 xs:w-44 md:h-36 md:w-96" alt="" />
    <div :class="props.edit && 'flex-col items-center mx-auto'" class="flex">
      <h1 v-if="props.edit" class="text-[#DDCCAA] font-semibold my-2 xs:text-xs md:text-xl">
        {{ $t('add_movie.replace_photo') }}
      </h1>
      <div class="flex items-center">
        <PhotoIcon :class="props.edit && 'xs:hidden md:flex'" class="mr-3 xs:w-6 md:w-auto" />
